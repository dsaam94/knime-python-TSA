...
class AggregationGranularity:
    ...
    def configure(self, configure_context:knext.ConfigurationContext, input_schema_1):
        ...
        self.aggreg_params.datetime_col = kutil.column_exists_or_preset(
            configure_context, self.aggreg_params.datetime_col, input_schema_1, kutil.is_type_timestamp
        )
        

        #set aggregation column
        self.aggreg_params.aggregation_column = kutil.column_exists_or_preset(
            configure_context, self.aggreg_params.aggregation_column, input_schema_1, kutil.is_numeric
        )
    ...
    def execute(self, exec_context: knext.ExecutionContext, input_1):
        ...
        df_grouped = df_grouped[[self.aggreg_params.datetime_col, self.aggreg_params.aggregation_column]]
    ...
    def __modify_time(self,time_gran:str,kn_date_time_type:str,df_time:pd.DataFrame):
        ...
        if (time_gran in (self.aggreg_params.TimeGranularityOpts.YEAR.name.lower(),
        ...
        # round datetime to nearest hour
            ...
            df[self.aggreg_params.datetime_col] = self.__floor_time(kn_date_time_type,"H", date)
        ...
        # round datetime to nearest minute
            ...
            df[self.aggreg_params.datetime_col] = self.__floor_time(kn_date_time_type,"min", date)
        ...
        # round datetime to nearest second. This option is feasble if timestamp contains milliseconds/microseconds/nanoseconds.
            ...
            df[self.aggreg_params.datetime_col] = self.__floor_time(kn_date_time_type,"S", date)
 
        
        return df
        
    def __floor_time(self, kn_date_time_type:str, time_gran:str, date:pd.Series) -> pd.Series:
    ...
    def __aggregate(self, df_time:pd.Series, aggregation_column:pd.Series, time_gran:str, agg_type:str):
        ...
        df = df[[self.aggreg_params.datetime_col, self.aggreg_params.aggregation_column, time_gran]]

        #select granularity 
        value = self.aggreg_params.AggregationDictionary[agg_type.upper()].value[1]

        # aggregate for final output
        df = df.groupby([df[self.aggreg_params.datetime_col], time_gran]).agg(value).reset_index()
        return df
    ...
    def __append_time_zone(self, date_col:pd.DataFrame, zoned:pd.Series):
        ...
        for i in range(0,len(zoned.index)):
            date_col_internal[self.aggreg_params.datetime_col][i] = date_col_internal[self.aggreg_params.datetime_col][i].replace(tzinfo=zoned.iloc[i])
